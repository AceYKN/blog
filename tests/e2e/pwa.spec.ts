...
async function getDistNoteCandidates() {
  const catalog = JSON.parse(await readFile(resolve('dist/search-catalog.json'), 'utf8')) as { documents?: NoteCandidate[] }
  return (catalog.documents || [])
    .filter((document) => document.kind === 'notes' && document.path && document.url && !document.path.endsWith('/index'))
    .slice(0, 2)
}
...
function serverBaseURL() {
  return `http://127.0.0.1:4173${normalizeBaseURL(process.env.NUXT_APP_BASE_URL || '/')}`
}

function serverNoteURL(note: NoteCandidate) {
  return new URL(`.${note.url}`, serverBaseURL()).href
}
...
test.describe('AceYKN PWA', () => {
  ...
  test('caches the first note before it is reloaded offline', async ({ page, context }) => {
    const notes = await getDistNoteCandidates()
    expect(notes.length).toBeGreaterThanOrEqual(1)
    const firstNote = notes[0]
    const firstURL = serverNoteURL(firstNote)

    await page.goto(firstURL)
    const title = await page.locator('article h1').innerText()
    await waitForServiceWorker(page)
    await expect(page.getByText('已可離線閱讀目前內容')).toBeVisible()
  ...
  test('reads accessed notes while offline', async ({ page, context }) => {
    await page.goto('./')
    await waitForServiceWorker(page)
  ...
  test('shows the offline fallback for an unvisited route', async ({ page, context }) => {
    ...
    await expect(page.getByRole('link', { name: '返回首頁' })).toHaveAttribute(
      'href',
      normalizeBaseURL(process.env.NUXT_APP_BASE_URL || '/')
    )
  })

  test('does not turn missing static resources into the HTML shell', async ({ request }) => {
    const baseURL = serverBaseURL()
    for (const missingAsset of ['_nuxt/__never-built__.js', '__never-built__.png']) {
      const response = await request.get(new URL(missingAsset, baseURL).href)
      expect(response.status(), missingAsset).toBe(404)
      expect(response.headers()['content-type'] || '', missingAsset).not.toContain('text/html')
    }

    const missingRoute = await request.get(new URL('notes/__never-cached-by-aceykn-pwa__', baseURL).href, {
