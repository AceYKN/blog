...
async function collect(directory) {
  ...
  const files = await Promise.all(entries.map(async (entry) => entry.isDirectory() ? collect(join(directory, entry.name)) : [join(directory, entry.name)]))
  return files.flat()
}
