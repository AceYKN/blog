...
async function resolveFile(pathname, request) {
  ...
  const isNavigationRequest = !extname(scopedPath) && acceptsHTML
  ...
  return isNavigationRequest ? join(root, 'index.html') : null
}
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    const filePath = await resolveFile(requestURL.pathname, request)
    if (!filePath) {
